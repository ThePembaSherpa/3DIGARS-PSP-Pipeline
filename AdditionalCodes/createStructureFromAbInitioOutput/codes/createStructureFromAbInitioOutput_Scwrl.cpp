...
void createPDBFromChromosomeAfterEachGeneration(Genome &geno, string id, int generations, int i, string fastaSeq, int amino_start_index, int chromo_len);
	...
	if (readFile.is_open()){
		...
		while (!readFile.eof()){
			...
			for (int i = 0; i < pop_size; i++){
				...
				createPDBFromChromosomeAfterEachGeneration(geno, id, generation, i, fastaSeq, amino_start_index, chromo_len);
...
void createPDBFromChromosomeAfterEachGeneration(Genome &geno, string id, int generation, int chromo_index, string fastaSeq, int amino_start_index, int chromo_len){
	...
	*/
	...
	/*
	string oscarScriptFile = "../oscar-star2/runOscarStar2_T0981";
