...
void createPDBFromChromosomeAfterEachGeneration(Genome &geno, string id, int generations, int i, string fastaSeq, int amino_start_index, int chromo_len, string opPath);
...
int main(int argc, char* argv[]){
	...
	string createDirCmd = "mkdir ../../../output/" + id;
	...
	string opPath = "../../../output/" + id+"/";

	/* read the fasta file and parse fasta line */
	string fastaFilePath = "../../../" + id + "/"+id+".fasta";
	...
	int pop_size = 0;
	string outFilePath = "../../../3DIGARS-PSP/" + id +"/"+ id +"_out.txt";
	...
	if (readFile.is_open()){
		...
		while (!readFile.eof()){
			...
			for (int i = 0; i < pop_size; i++){
				...
				createPDBFromChromosomeAfterEachGeneration(geno, id, generation, i, fastaSeq, amino_start_index, chromo_len, opPath);
...
void createPDBFromChromosomeAfterEachGeneration(Genome &geno, string id, int generation, int chromo_index, string fastaSeq, int amino_start_index, int chromo_len, string opPath){
	...
	string opBackbonePdbFile = opPath + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_bb.pdb";
	...
	string scwrlInputPdb = "../../output/" + id + "/" + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_bb.pdb";
	...
	*/
	...
	string oscarScriptFile = "../../../3DIGARS-PSP/oscar-star2/runOscarStar2_"+id;
	...
	if (oscarScript.is_open()){
		...
		oscarScript << "\n";
		oscarScript << "cd ../../../3DIGARS-PSP/oscar-star2/\n";
	...
	string oscarOutputPdb = "../../../3DIGARS-PSP/oscar-star2/"+id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_bb_model.pdb";
	string copyOscarOp = "mv -f " + oscarOutputPdb + " " + opPath + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_full.pdb";
	...
	string pdbFilePath = opPath + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_full.pdb";
	ifstream pdbFileStream(pdbFilePath.c_str());
	string linePdbFile;

	string opPdbFilePath = opPath + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + ".pdb";
	...
	string removefullatomicmodel = "rm -f " + pdbFilePath;
	...
	// remove the backbone model file
	scwrlInputPdb = opPath + id + "_" + NumberToString(generation) + "_" + NumberToString(chromo_index) + "_bb.pdb";
	string removebbmodel = "rm -f " + scwrlInputPdb;
	sysRet = system(removebbmodel.c_str());
	if (sysRet == -1){
