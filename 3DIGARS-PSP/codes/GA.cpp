...
//	string rama_phipsi_file = "../rama-data/phiPsiFreq.txt";
	...
	if (rank == 0){
		...
		int sysRet = system(listPdbFiles.c_str());
		if (sysRet == -1){
		...
		string list_model_files_path = "../input/seeds/input/list.txt";
		ifstream listStream(list_model_files_path.c_str());
		string listLine;
		if (listStream.fail()){

			cout << "Could not open file " << list_model_files_path << "\n";
			exit(EXIT_FAILURE);
		...
		if (listStream.is_open()){
			...
			getline(listStream, listLine); // get the first model's name from the list file and use it to obtain the chromo_len and amino_start_ind
			listStream.close();
		}


		// read input pdb file and obtain chromosome length
		string pdbFilePath = "../input/seeds/input/" + listLine;
	...
	// MPI_Comm_spawn is a collective function so it can be only called for all processes
	MPI_Bcast(&chromo_len, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&amino_start_ind, 1, MPI_INT, 0, MPI_COMM_WORLD);
...
//	string ssFilePath = "../rama-data/ssFreq.txt";
	...
	if(rank == 0){
		...
		*/
		...
		outfile << "amino_start_index\t" << amino_start_ind << endl;
...
//			}
		...
		//	Genome genoTemp1;
			...
			*/
			...
			// read a file to update the max_generations then broadcast it to all other processes by calling MPI_Bcast
			string max_gen_file_path = "../input/max_generation.txt";
			ifstream maxGenStream(max_gen_file_path.c_str());
			string max_gen_value;
			
			if (maxGenStream.is_open()){

				getline(maxGenStream, max_gen_value);
				max_generations = atoi(max_gen_value.c_str());
				maxGenStream.close();
			}
		...
		MPI_Bcast(&max_generations, 1, MPI_INT, 0, MPI_COMM_WORLD);
...
//				getline(pdbStream, pdbLine);	// skip the line that starts with MODEL 1
			...
			//	string chain;
				...
				//		str >> atom_num >> atom_type >> amino_acid >> chain >> amino_num >> x >> y >> z;
				//		str >> atom_num >> atom_type >> amino_acid >> amino_num >> x >> y >> z;
						atom_type = removeSpaces(pdbLine.substr(13, 4));
						
						
						if(strcmp(atom_type.c_str(), "N") == 0 || strcmp(atom_type.c_str(), "CA") == 0 || strcmp(atom_type.c_str(), "C") == 0 || strcmp(atom_type.c_str(), "O") == 0){
						//	opStream << atom_type << endl;
						...
						//	Zcor[pdbLineCounter] = z;
							...
							x = strtof(pdbLine.substr(31, 8).c_str(), &xEnd);
							y = strtof(pdbLine.substr(39, 8).c_str(), &yEnd);
							z = strtof(pdbLine.substr(46, 8).c_str(), &zEnd);
